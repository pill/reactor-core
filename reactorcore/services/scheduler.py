...
class SchedulerService(RedisSource, BaseService):
    ...
    def __init__(self):
        super(SchedulerService, self).__init__(
            name="SCHEDULER", cls=self.__class__
        )
        self.initialize()
        self._locks = {}
    ...
    def find_ready_tasks(self):
        ...
        logger.debug(
            "Redis time: %s %s %s:%s", n.month, n.day, n.hour, n.minute
        )
        ...
        for task in conf["cron"]["tasks"]:
            ...
            if all(
                [
                    self._is_ready(t, n_times[count])
    ...
    def _acquire_lock(self, task):
        ...
        if task["name"] in self._locks:
            ...
            task_lock = self.client.lock(
                task["name"], timeout=min_ttl, sleep=1
            )
    ...
    def _release_lock(self, *args):
        ...
        if total_run_time < min_ttl:
            logger.debug(
                "Skip releasing lock ended(%s) < min_ttl(%s)",
                end_time,
                min_ttl,
            )
        else:
    ...
    def run_task_process(self, task, callback):
        try:
            ...
            logger.critical(
                "[EXCEPTION] Could not create subprocess, %s: %s",
                stacktrace,
                ex.message,
                exc_info=True,
            )
    ...
    def _get_callable(self, task_name):
        ...
        if not task_info:
            raise Exception(
                "No task defined for task_name: `{}`".format(task_name)
            )
