...
map(lambda logger: logger.update({'handlers': ['syslog']}), SYSLOG['loggers'].values())
SYSLOG['loggers']['reactorcore.dao']['level'] = 'WARNING'

# both
CONSOLE_AND_SYSLOG = copy.deepcopy(BASE_CONFIG)
map(lambda logger: logger.update({'handlers': ['console', 'syslog']}), CONSOLE_AND_SYSLOG['loggers'].values())

ALL = {Env.INT: CONSOLE_AND_SYSLOG,
       Env.PROD: SYSLOG,
