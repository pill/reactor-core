...
class BaseApiHandler(base.BaseRequestHandler):
    ...
    def set_default_headers(self):
        ...
        self.set_header(
            ...
            "Content-Type, X-NSQ-Auth, X-NSQ-Test",
        )
    ...
    def log_request(self):
        ...
        logger.info(
            "API request START [id:%s]  %s:%s; DATA: %s",
            self.request.id,
            self.request.method,
            self.request.uri,
            masked_args,
    ...
    def log_request_end(self):
        ...
        logger.info(
            "API request END [id:%s] %s, HTTP %s in %.2fms",
            _id,
            self.request.uri,
            self.get_status(),
    ...
    def write_error(self, status_code, **kwargs):
        ...
        if isinstance(ex, exception.NotFound):
            logger.warning(
                "API request WARNING [%s] for user %s: %s",
                self.request.id,
                self.current_user,
                ex,
            )
            ...
            response_data = {
                "form_error": {"message": constants.MSG_NOT_FOUND}
            }
        ...
        if ex:
            ...
            _id = self.request.id if hasattr(self.request, "id") else "(no id)"
            logger.critical(
                '[EXCEPTION] Request end %s %s %s [%s] for user %s. Error: %s. Occurred at: %s:%s %s() in "%s"',
                self.request.method,
                self.request.uri,
                self.get_status(),
                _id,
    ...
    def login(self):
        ...
        yield self.convert_pending_comments_and_replies(
            self.current_user, gsid
        )
    ...
    def _get_guest_session_id(self):
        ...
        guest_session_id = self.get_secure_cookie(
            constants.COOKIE_GUEST_SESSION_ID, max_age_days=10 * 365
        )
