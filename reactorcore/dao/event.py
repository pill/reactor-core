...
class EventDao(redis.RedisSource):
    ...
    def create_event(self, e, group_by=None):
        ...
        if group_by:
            ...
            """
            group = "event:group:{}-{}-{}".format(str(group_by), event.handler, event.ready_after)

            existing_score = self.client.get(group)
            if existing_score:
                logger.debug(
                    'Events for group %s already exists with score %s', group, existing_score)
                event.score = existing_score
            else:
                event.score = time.time() + event.ready_after
                logger.debug('NEW score for events in group %s: %s', group, event.score)
        else:
    ...
    def pop_ready_events(self):
        ...
        for rec in data:
            ...
            if group:
                logger.debug('Adding group %s to the set of GROUPS TO BE DELETED', group)
